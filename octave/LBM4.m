...
fbak = zeros(Ny,Nx); %Temporary variable that stores the result of the collision process.
...
for t = 1:Tsim
    for k = 1:Nl
        %-------COLLISION-------
        fbak(:,:) = f(:,:,k)*(1-omega)+omega*feq(:,:,k);
        %-------STREAMING (including the periodic boundary conditions)-------
        for j = 1:Ny
            for i = 1:Nx
                newx = mod(i-1-ex(k)+Nx,Nx)+1; 
                newy = mod(j-1-ey(k)+Nx,Ny)+1;
                f(j,i,k) = fbak(newy,newx);
            end
        end
    end
